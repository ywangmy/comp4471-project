...
def extract_video(param, video_dir, crops_dir):
    ...
    for i in range(frames_num):
        ...
        for bbox in bboxes:
            ...
            h, w = crop.shape[:2]
            crops.append(crop)
        img_dir = os.path.join(video_dir, crops_dir, id)
...
def get_video_paths(video_dir):
    ...
    for json_path in glob(os.path.join(video_dir, "*/metadata.json")):
        ...
        for k, v in metadata.items():
            ...
            if not original:
                original = k
            bboxes_path = os.path.join(video_dir, "boxes", original[:-4] + ".json")
...
def main():
    ...
    os.makedirs(os.path.join(args.video_dir, args.crops_dir), exist_ok=True)
    params = get_video_paths(args.video_dir)
    with Pool(processes=cpu_count()) as p:
        with tqdm(total=len(params)) as pbar:
            for v in p.imap_unordered(partial(extract_video, video_dir=args.video_dir, crops_dir=args.crops_dir), params):
                pbar.update()
